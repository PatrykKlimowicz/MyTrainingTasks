...
int main(int argc, char *argv[])
{
    ...
    for (l = 0; l < 10; l++ ){
        ...
        for (k = 0; k < NUM_LOOPS; k++) {
            start_1 = PAPI_get_real_usec();
            start_2 = PAPI_get_real_usec();
            ...
            for (i = 0; i < rows; i++) {
                end_1 = PAPI_get_real_usec();
                v[i] = 0.0f;
                for (j = 0; j < columns; j++)
                    v[0] += v[i] + A[i][j] * u[j];
            }
            end_2 = PAPI_get_real_usec();
            printf("wartosc obliczona: %f\n", v[0]);
            end_3 = PAPI_get_real_usec();

            printf("CZAS 1: %f\n", (float)( end_1 - start_1) / 1000000);
            printf("CZAS 2: %f\n", (float)( end_2 - start_2) / 1000000);
            printf("CZAS 3: %f\n", (float)( end_3 - start_3) / 1000000);
            printf("CZAS DLA PRINTF: %f\n", (float)( end_3 - start_3) / 1000000 - (float)( end_2 - start_2) / 1000000);
        }
